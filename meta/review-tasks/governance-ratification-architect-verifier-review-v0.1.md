...
**Status:** DRAFT pre-stage (provisional RT-2026-006).
...
- **(c) §3 revised with stricter compliance requirement.** The four trackers must add proper per-entry document-control sections. (Architect: please weigh whether this is operationally feasible given the entry rate.)
...
Both are operationally primary in their jurisdictions, not merely advisory. The "advisory flags" wording understates their role and creates a potential rescue surface (a future stance-discipline disagreement could try to discount a Scout or Verifier ruling on the basis that they are "only advisory").
...
- **(c) §4 left as drafted.** Architect rules that "advisory" remains accurate and prior Scout/Verifier rulings stand on the basis of Council ratification, not stance authority. (This carries the rescue surface noted above.)

Architect ruling required.

### Q3 — §5 (Endorsement Marker) consistency check across in-tree artefacts
...
**Issue.** Spot-check requested across:
...
Architect ruling required. If non-compliance is found, list affected files and prescribe remediation.
...
- **Q5(c)** Are there other implicit conventions in current operational practice that should be codified? Architect free-form audit invited.
...
**Issue.** FAIR.md operationalises the FAIR principles as per-commit and per-release checklists. Spot-check whether the realised repository state matches the operational claims:
- **F**indability: "every dataset carries a schema." Current state: `data/m1-catholic-councils/` and `data/t10-archive/` carry schemas; `data/defunct-archives/` does not (it is a placeholder per its own README). Is the operational claim violated, or is "every dataset" implicitly limited to non-placeholder datasets?
...
Architect ruling required, with prescribed remediation where the operational claim and the realised state diverge.
...
**Ruling options:**
- **(a) Checklists ratified as binding.** Pre-commit and pre-release checklist completion required; non-completion blocks the commit/release. (Operationally heavy.)
...
1. **Endorsement Markers update.** Both files' Endorsement Markers move from `Stance: Guardian (drafting). Reviewed by: pending Architect/Verifier.` to `Stance: Guardian (drafting). Reviewed by: Architect [date], Verifier [date].` with any required modifications applied.
2. **Modifications applied.** Any §3 carve-out, §4 reclassification, §5 endorsement-marker remediation, §6 non-prescription-clause clarification, §7+ new conventions, FAIR-checklist revisions, citation corrections, or license-marking changes ruled by Architect or Verifier are drafted into the files.
3. **Council decision lodged.** A new `meta/council-decisions.md` entry (numbering: 2026-017 if no other decisions have intervened) records: stance composition, brief reference (RT-2026-006), per-question rulings summarised, modifications applied, Endorsement Marker status updated, audit-observation closure.
4. **Stance log updated.** Architect issuance entry and Architect/Verifier response entries appended to `meta/stance-log.md`.
5. **CHANGELOG release.** Tier-5 closure released as v0.3.0 (MINOR bump: substantive convention modifications and Endorsement Marker upgrades qualify as added functionality in the doc-as-functionality sense). New tag, CITATION.cff and README closing-line bump.
6. **Outstanding clear.** CHANGELOG `[0.2.2]` Outstanding section item ("CONVENTIONS.md and FAIR.md remain in 'Stance: Guardian (drafting). Reviewed by: pending Architect/Verifier'") moved to Resolved in the v0.3.0 entry.

---

## §8. Process and scheduling

- **Pre-issuance.** This brief is at DRAFT pre-stage. Issuance requires user/Council confirmation. Architect smell-test review optional (pattern matches RT-2026-004 / RT-2026-005 issuance discipline; given that this brief is itself drafted in Architect voice, smell-test is degenerate and may be skipped).
- **Issuance.** On user confirmation, brief moves DRAFT pre-stage → ACTIVE; provisional RT-2026-006 confirmed; entry lodged in `meta/review-tasks/README.md` and `meta/stance-log.md`; Council decision (2026-017 or next) lodged at `meta/council-decisions.md` recording issuance.
...
- **Cross-binding to tier-6.** Tier-6 substantive methodological concerns (Inverse-R4 fragility, R8/R3 status, TC-05 dependency scoping) are downstream of TC-card work and out of scope for this brief. Tier-5 ratification must complete before tier-6 deliberation begins, on the principle that tier-6 will issue rulings under conventions; the conventions themselves should be ratified first.
...
- **v0.1** (DRAFT pre-stage, 2026-04-29): Initial brief drafted by Architect on behalf of Council to close the tier-5 audit observation. Five Architect questions (Q1–Q5) on CONVENTIONS.md scope and §3 carve-out / §4 stance classification / §5 endorsement-marker compliance / §6 non-prescription clause / §7+ missing conventions; two Architect questions (Q6–Q7) on FAIR.md operational claims and checklist binding-status; three Verifier questions (Q8–Q10) on external-citation accuracy / license-triple compliance / standards-compliance claims. Cross-binding to tier-4 deferred §3 carve-out installed as Q1 binding context. Per-jurisdiction split permitted. Net-new risk catch-all flags ratification-cascade risks with prospective-only mitigation.
- **Pending:** User/Council confirmation for issuance. On issuance, status moves DRAFT pre-stage → ACTIVE; RT-2026-006 confirmed; logged in council-decisions, stance-log, and meta/review-tasks/README.md.

**Stance log.** Drafted Architect. Brief structure follows the RT-2026-002 / RT-2026-004 / RT-2026-005 pattern (binding-ruling questions, cross-binding mandates, per-jurisdiction split provision, net-new risk catch-all, deliverable specification, document control). No Guardian veto issued at draft. Integrator decision required on issuance.

---

*Architect-stance brief. Provisional RT-2026-006. ~2400 words. DRAFT pre-stage pending user confirmation for issuance.*
